...
* getRecUser
*/
exports.getRecUser = function(res,uid){
    var queryDocument = function(db,callback){
        var arr1 =[],
            arr2 =[],
            arrResult = [];
        db.collection('users').find().toArray().then(function(rs){
            var i = 0;
            ...
            db.collection('fellowing').find({"uId":uid}).toArray().then(function(rs){
                var i = 0;
                var rd = rs[0].fellowingList;
                for(i=0;i<rd.length;i++){
                    arr2.push(rd[i].uId);
                }
                var arrResult = diff_array(arr1,arr2);
                var resArr = [];

                for(i=0;i<arrResult.length;i++){
                    console.log("anow:",arrResult[i]);
                    db.collection('users').find({"uId":arrResult[i]}).toArray().then(function(rs){
                        resArr.push(rs[0]);
                        console.log("rss:",rs);
                        if(resArr.length==arrResult.length){
                            httpRes.sendSuccess_nc(res,{"code":"0","msg":"获取推荐好友成功!","friendsList":resArr});
                        }
                    })
                }
            });
        });
        // httpRes.sendSuccess_nc(res,{"code":"0","msg":"获取推荐好友成功!","friendsList":arrResult});
    };
